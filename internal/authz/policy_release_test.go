...
func TestPolicyFixtureReleaseBuilds(t *testing.T) {
	...
	if len(ledger.Records) != 1 || ledger.LedgerHash == "" {
		t.Fatal(ledger)
	}
	...
	if review.Status != PolicyFixtureReleaseOK || review.RecordCount != 1 || review.ReviewHash == "" {
		t.Fatal(review)
	}
}
...
func TestPolicyFixtureReleaseHashIsDeterministic(t *testing.T) {
	...
	if left.ReleaseHash != right.ReleaseHash {
		t.Fatalf("%s != %s", left.ReleaseHash, right.ReleaseHash)
	}
}
