...
func PolicyFixtureMarkerLogFromRecords(records []PolicyFixtureMarkerRecord) (PolicyFixtureMarkerLog, error) {
	...
	if err != nil {
		return PolicyFixtureMarkerLog{}, err
	}
	if len(normalized) == 0 {
		return PolicyFixtureMarkerLog{}, fmt.Errorf("%w: marker records are required", ErrInvalidPolicyFixtureRelease)
	}
	...
	if err := ValidatePolicyFixtureMarkerLog(log); err != nil {
		return PolicyFixtureMarkerLog{}, err
	}
	return log, nil
}
...
func NormalizePolicyFixtureMarkerRecords(records []PolicyFixtureMarkerRecord) ([]PolicyFixtureMarkerRecord, error) {
	...
	for _, record := range records {
		...
		if existing, ok := seen[record.MarkerHash]; ok {
			if existing.ReleaseHash != record.ReleaseHash || existing.LedgerHash != record.LedgerHash {
				return nil, fmt.Errorf("%w: conflicting marker record", ErrInvalidPolicyFixtureRelease)
			}
			continue
		}
...
func ValidatePolicyFixtureMarkerLog(log PolicyFixtureMarkerLog) error {
	if log.SchemaVersion != PolicyFixtureMarkerLogSchemaVersion || !log.FixtureOnly {
		return fmt.Errorf("%w: invalid marker log metadata", ErrInvalidPolicyFixtureRelease)
	}
	if len(log.Records) == 0 {
		return fmt.Errorf("%w: marker log records are required", ErrInvalidPolicyFixtureRelease)
	}
	if _, err := NormalizePolicyFixtureMarkerRecords(log.Records); err != nil {
		return err
	}
	if !validSHA256Hex(log.LogHash) || PolicyFixtureMarkerLogHash(log) != log.LogHash {
		return fmt.Errorf("%w: marker log hash mismatch", ErrInvalidPolicyFixtureRelease)
	}
	return nil
}
...
func PolicyFixtureMarkerLogHash(log PolicyFixtureMarkerLog) string {
	...
	for _, record := range records {
		parts = append(parts, record.MarkerHash)
	}
