...
func PolicyFixtureReleaseLedgerFromRecords(records []PolicyFixtureReleaseRecord) (PolicyFixtureReleaseLedger, error) {
	...
	if err != nil {
		return PolicyFixtureReleaseLedger{}, err
	}
	if len(normalized) == 0 {
		return PolicyFixtureReleaseLedger{}, fmt.Errorf("%w: release records are required", ErrInvalidPolicyFixtureRelease)
	}
...
func NormalizePolicyFixtureReleaseRecords(records []PolicyFixtureReleaseRecord) ([]PolicyFixtureReleaseRecord, error) {
	...
	for _, record := range records {
		if err := ValidatePolicyFixtureReleaseRecord(record); err != nil {
			return nil, err
		}
		if existing, ok := seen[record.ReleaseHash]; ok {
			if existing.BundleHash != record.BundleHash || existing.ManifestHash != record.ManifestHash {
				return nil, fmt.Errorf("%w: conflicting release record", ErrInvalidPolicyFixtureRelease)
			}
			continue
		}
...
func ValidatePolicyFixtureReleaseLedger(ledger PolicyFixtureReleaseLedger) error {
	if ledger.SchemaVersion != PolicyFixtureReleaseLedgerSchemaVersion || !ledger.FixtureOnly {
		return fmt.Errorf("%w: invalid release ledger metadata", ErrInvalidPolicyFixtureRelease)
	}
	if len(ledger.Records) == 0 {
		return fmt.Errorf("%w: release ledger records are required", ErrInvalidPolicyFixtureRelease)
	}
	if _, err := NormalizePolicyFixtureReleaseRecords(ledger.Records); err != nil {
		return err
	}
	if !validSHA256Hex(ledger.LedgerHash) || PolicyFixtureReleaseLedgerHash(ledger) != ledger.LedgerHash {
		return fmt.Errorf("%w: release ledger hash mismatch", ErrInvalidPolicyFixtureRelease)
	}
	return nil
}
...
func PolicyFixtureReleaseLedgerHash(ledger PolicyFixtureReleaseLedger) string {
	...
	for _, record := range records {
		parts = append(parts, record.ReleaseHash)
	}
