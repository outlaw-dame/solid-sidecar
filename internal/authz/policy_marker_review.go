...
func PolicyFixtureMarkerReviewForLog(log PolicyFixtureMarkerLog, checkedAtUnix int64) (PolicyFixtureMarkerReview, error) {
	if checkedAtUnix < 0 { return PolicyFixtureMarkerReview{}, fmt.Errorf("%w: negative marker review time", ErrInvalidPolicyFixtureRelease) }
	status := PolicyFixtureMarkerOK
	if ValidatePolicyFixtureMarkerLog(log) != nil { status = PolicyFixtureMarkerFailed }
	...
	if err := ValidatePolicyFixtureMarkerReview(review); err != nil { return PolicyFixtureMarkerReview{}, err }
	return review, nil
}

func ValidatePolicyFixtureMarkerReview(review PolicyFixtureMarkerReview) error {
	if review.SchemaVersion != PolicyFixtureMarkerReviewSchemaVersion || !review.FixtureOnly { return fmt.Errorf("%w: invalid marker review metadata", ErrInvalidPolicyFixtureRelease) }
	if review.Status != PolicyFixtureMarkerOK && review.Status != PolicyFixtureMarkerFailed { return fmt.Errorf("%w: invalid marker review status", ErrInvalidPolicyFixtureRelease) }
	if !validSHA256Hex(review.LogHash) || !validSHA256Hex(review.ReviewHash) || review.RecordCount < 0 || review.CheckedAtUnix < 0 { return fmt.Errorf("%w: invalid marker review values", ErrInvalidPolicyFixtureRelease) }
	if PolicyFixtureMarkerReviewHash(review) != review.ReviewHash { return fmt.Errorf("%w: marker review hash mismatch", ErrInvalidPolicyFixtureRelease) }
	return nil
}
