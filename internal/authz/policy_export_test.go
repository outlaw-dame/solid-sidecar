...
func TestPolicyFixtureExportBuilds(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(index.Records) != 2 || index.IndexHash == "" || !index.FixtureOnly { t.Fatal(index) }
	check, err := PolicyFixtureExportCheckForIndex(index, 200)
	if err != nil { t.Fatal(err) }
	if check.Status != PolicyFixtureExportOK || check.RecordCount != 2 || check.CheckHash == "" { t.Fatal(check) }
}
...
func TestPolicyFixtureExportHashesAreDeterministic(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if left.ExportHash != right.ExportHash { t.Fatalf("%s != %s", left.ExportHash, right.ExportHash) }
}
