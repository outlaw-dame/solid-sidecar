...
func TestMiddlewareFallsBackAfterEvaluatorError(t *testing.T) {
	var logs bytes.Buffer
	logger := slog.New(slog.NewJSONHandler(&logs, &slog.HandlerOptions{Level: slog.LevelDebug}))
	...
	fallback := &fakeEvaluator{decision: fallbackDecision}
	nextCalled := false
	next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		nextCalled = true
		w.WriteHeader(http.StatusAccepted)
	...
	if !primary.seen || !fallback.seen {
		t.Fatalf("expected both evaluators to be called: primary=%v fallback=%v", primary.seen, fallback.seen)
	}
	if !nextCalled {
		t.Fatal("expected next handler to be called")
	}
	...
	assertNotContainsAny(t, logOutput, `"error":`, "boom", "token=secret-token", "secret-token")
}
