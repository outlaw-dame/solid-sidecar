...
func TestPolicyFixtureMarkersBuild(t *testing.T) {
	...
	if len(log.Records) != 1 || log.LogHash == "" {
		t.Fatal(log)
	}
	...
	if markerReview.Status != PolicyFixtureMarkerOK || markerReview.RecordCount != 1 || markerReview.ReviewHash == "" {
		t.Fatal(markerReview)
	}
}
...
func TestPolicyFixtureMarkerHashIsDeterministic(t *testing.T) {
	...
	if record.MarkerHash != copy.MarkerHash {
		t.Fatalf("%s != %s", record.MarkerHash, copy.MarkerHash)
	}
