...
func PolicyFixtureReleaseReviewForLedger(ledger PolicyFixtureReleaseLedger, checkedAtUnix int64) (PolicyFixtureReleaseReview, error) {
	if checkedAtUnix < 0 { return PolicyFixtureReleaseReview{}, fmt.Errorf("%w: negative release review time", ErrInvalidPolicyFixtureRelease) }
	status := PolicyFixtureReleaseOK
	if ValidatePolicyFixtureReleaseLedger(ledger) != nil { status = PolicyFixtureReleaseFailed }
	...
	if err := ValidatePolicyFixtureReleaseReview(review); err != nil { return PolicyFixtureReleaseReview{}, err }
	return review, nil
}

func ValidatePolicyFixtureReleaseReview(review PolicyFixtureReleaseReview) error {
	if review.SchemaVersion != PolicyFixtureReleaseReviewSchemaVersion || !review.FixtureOnly { return fmt.Errorf("%w: invalid release review metadata", ErrInvalidPolicyFixtureRelease) }
	if review.Status != PolicyFixtureReleaseOK && review.Status != PolicyFixtureReleaseFailed { return fmt.Errorf("%w: invalid release review status", ErrInvalidPolicyFixtureRelease) }
	if !validSHA256Hex(review.LedgerHash) || !validSHA256Hex(review.ReviewHash) || review.RecordCount < 0 || review.CheckedAtUnix < 0 { return fmt.Errorf("%w: invalid release review values", ErrInvalidPolicyFixtureRelease) }
	if PolicyFixtureReleaseReviewHash(review) != review.ReviewHash { return fmt.Errorf("%w: release review hash mismatch", ErrInvalidPolicyFixtureRelease) }
	return nil
}
