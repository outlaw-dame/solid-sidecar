...
Both Go and Rust tests read `authz_manifest.json`. Changes to request or decision fixture files must also update the manifest and should be reviewed carefully. Go tests audit the fixture directory so orphan `authz_request.*.json` and `authz_decision.*.json` files fail fast. Go and Rust tests both validate manifest schema version, duplicate entries, local fixture filename prefixes, and path-separator rejection.
